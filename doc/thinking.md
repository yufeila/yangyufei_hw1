 ...
 * @Date: 2024-05-05 11:46:37
 * @LastEditors: error: error: git config user.name & please set dead value or install git && error: git config user.email & please set dead value or install git & please set dead value or install git
 * @LastEditTime: 2024-05-10 10:04:25
 * @FilePath: \yangyufei_hw1\doc\thinking.md
...
2. 接下来，函数将矩阵转化为上三角形式。这个过程通过高斯消元法实现，具体步骤如下：
   ...
   d. 如果在下方找不到非零元素，说明当前列已经全为零，需要将秩减一，并将当前列的元素全部设为最后一列对应位置的元素。如果单纯通过互换列，会出现一些问题，这里不建议这么操作。而是在原来矩阵的基础上跳过这一列，不做处理，直接进入下一列。
   
   e.操作时，应该先将全0行移到矩阵的末尾。
